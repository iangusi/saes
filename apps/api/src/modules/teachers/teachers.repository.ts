...
export class TeachersRepository {
  ...
  console.log('>>> buscando profesor para id_usuario:', idUsuario);    
  ...
  async getTeacherSchedule(idProfesor: number): Promise<TeacherSchedule> {
    console.log('>>> getTeacherSchedule llamado con idProfesor:', idProfesor); // ← AGREGAR

      const grupos = await this.getTeacherGroups(idProfesor);
      console.log('>>> grupos encontrados:', JSON.stringify(grupos));            // ← AGREGAR
    ...
    return { grupos, horarios: rows as any[] };
  }
  ...
  // BUG FIX: capturada_por usa idUsuario del token, no un id_profesor hardcodeado
  async updateGrade(
    ...
    const sql = `
      INSERT INTO calificacion
        (id_inscripcion, id_grupo_evaluacion, calificacion, capturada_por, fecha_captura)
    ...
    `;
    await query(sql, [idInscripcion, idGrupoEvaluacion, calificacion, idUsuario]);
  }
  ...
  async recordAttendance(
    ...
    const inscripciones = await query(
      'SELECT id_inscripcion, id_alumno FROM inscripcion WHERE id_grupo = ? AND estatus = "activa"',
      [idGrupo]
    ) as any[];

    const values = asistencias.map((asist) => {
      const inscripcion = inscripciones.find((i) => i.id_alumno === asist.idAlumno);
      if (!inscripcion) throw new Error(`Alumno ${asist.idAlumno} no inscrito en el grupo`);
      return [
    ...
    const sql = `
      INSERT INTO asistencia
        (id_inscripcion, id_grupo, fecha, presente, justificada, registrada_por)
      VALUES ?
      ON DUPLICATE KEY UPDATE
  ...
  // ── anuncios ─────────────────────────────────────────────────────
  // BUG FIX: enviado_por referencia a usuario.id_usuario (no a profesor.id_profesor)
  ...
  async getGroupAnnouncements(idGrupo: number): Promise<any[]> {
    const sql = `
      SELECT
        ...
        DATE_FORMAT(a.fecha_creacion, '%Y-%m-%d %H:%i') as fechaCreacion
      FROM anuncio a
